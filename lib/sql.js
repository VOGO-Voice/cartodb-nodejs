...
var Stream = require('stream')
  ...
  , debug = require('debug')('cartodb:sql')
...
SQL.prototype.execute = function(sql, vars, options, cb) {
  ...
  if(!sql.match) {
    debug(sql)
    throw new Error('sql match does not exist');
  }
    ...
    try {
      ...
      if(!err && res.statusCode === 200) {
        ...
        if (err) {
          ...
          error = (body.error) ? body.error : JSON.stringify(body);
          debug('There was an error with the request %s', error);
        }
        promise.emit('_error', new Error(error));
      }
    } catch (err) {
      promise.emit('_error', new Error(err));
    }
  }
